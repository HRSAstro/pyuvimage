...
It is the quadrature sum of two terms, both written to the header as `ERRSTAT`
and `ERRSYS`:
...
identically. In all of them the point sources are injected **analytically** at
sub-pixel positions, so nothing in the truth sits on the model's grid: a point
placed on the truth image is a source the pixelized model can already
represent, and recovering it would prove nothing.
...
|---|---|---|---|---|
| crowded field | 1.000 | x1.000 | 4/4 | 0 |
| coarse beam, b_max 300 m (beam 1.13") | 1.159 | x0.998 | 0/4 * | 0 |
| nominal, b_max 800 m (beam 0.48") | 1.000 | x1.000 | 3/4 | 0 |
| fine beam, b_max 2.5 km (beam 0.16") | 0.999 | x0.996 | 3/4 | 1 |
| wide field, 8" instead of 4" | 1.000 | x0.998 | 4/4 | 0 |
| high S/N (sigma 3e-5) | 1.000 | x1.000 | 4/4 | 0 |
| nominal S/N (sigma 3e-4) | 1.000 | x1.000 | 4/4 | 0 |
| low S/N (sigma 1.5e-3) | 1.000 | x0.997 | 4/4 | 0 |
| **fov 2" for a 3" field** | **365** | x0.924 | 0/4 | fit refused |

`*` not a failure: with a 1.13" beam and 0.2" mesh pixels the mesh represents
an unresolved source perfectly well, so a delta component buys nothing and the
detector correctly finds none. Total flux is still recovered to 0.2% — the
point flux simply lives in the mesh.

**Total flux is recovered to 0.4% or better in every converged case.** All 26
detections land within 3 sigma of the truth, and positions within 0-17 mas
(a beam is 155-1130 mas). Detection is conservative by construction: 2 of the
28 recoverable points were missed and only 1 false positive appeared, a
0.17 mJy 6 sigma component 0.2" from a 12 mJy point in the one configuration
with more model pixels than data (2304 vs 1600).

Four real defects came out of this, all fixed:
...
3. **Components survived on a pre-retune significance.** The retune changes
   the errors, so the cut is applied again afterwards — that is what removes
...
emission is unrecoverable: chi^2/N runs to 365, and the point fitter, given a
residual that is model error rather than sky, previously returned an 11.5 Jy
"source" at 76 sigma in a 0.09 Jy field. Point fitting is now refused
outright when the pixelized model has not converged to its target, and the run
warns that no product should be trusted. Fix `--fov` first.

### Reading the residual map: a residual in sigma means nothing on its own

`pyuvimage demo` leaves an 8 sigma residual at the centre of the source, and
that is not a sign of a bad fit. A regularised model is smoothed, so it
...
**1095 sigma**, so an 8.3 sigma residual is an 0.76% error on it. Every run
now prints this explicitly:

```
peak brightness 0.00632 Jy/beam = 1095 sigma; largest residual 8.3 sigma
  = 0.76% of the peak (dynamic range 131:1)
```

and `summary.png` states it in the residual panel's title. CLEAN behaves the
same way; it is what limited dynamic range looks like.

The obvious levers do not help, which is worth knowing before you reach for
them. On the demo source, going from 300 to 3000 visibilities moves the
residual only from 13.0 to 11.3 sigma; a finer mesh makes it *worse*
(4.4 -> 6.2 -> 7.1 sigma at mesh 32/48/64); halving or thirding the prior's
correlation length makes it worse again (10.8 -> 17.5 -> 17.2 sigma); and a
Gaussian source instead of an exponential still leaves 10 sigma. The limit is
the smoothing prior itself, not resolution, sampling, or the source profile.

What *does* help is taking the compact flux out of the mesh entirely, which is
what `--point-sources` is for — and it is why the demo now uses it.
