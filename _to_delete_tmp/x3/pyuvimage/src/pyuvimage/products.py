...
def _summary_png(p: ProductSet, geometry: ImageGeometry, path: Path) -> None:
    ...
    # the model panel must show what model.fits holds, points included --
    # otherwise a fitted point simply vanishes from the summary
    model_panel = model_with_points(p, geometry.pixel_scale)
    n_pt = len(p.points or [])
    model_title = "model" + (f" (+{n_pt} point)" if n_pt else "")

    panels = [
        (p.dirty_image, "dirty image", "Jy/beam", "inferno", None),
        (model_panel, model_title, "Jy/pixel", "inferno", None),
