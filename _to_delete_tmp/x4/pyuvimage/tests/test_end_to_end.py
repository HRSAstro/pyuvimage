...
def test_default_prior_is_adaptive_power_two(tmp_path):
    """The default is the brightness-adaptive prior at power 2.

    It was `gibbs` briefly. Gibbs shortens the prior's correlation *length*
    where the source is bright, which strengthens the penalty there and
    suppresses the peak: it leaves a central residual that does not scale with
    source brightness (~9-10 sigma across a factor of 10 in flux). `adaptive`
    loosens the prior's *amplitude* instead and leaves ~1 sigma.
    """
    ...
    assert prior["regularization"] == "adaptive"
    assert prior["optimised"] is True
    assert fitting.ADAPT_POWER == 2.0
    assert res.parameters["source_prior"]["adaptive_power"] == 2.0
