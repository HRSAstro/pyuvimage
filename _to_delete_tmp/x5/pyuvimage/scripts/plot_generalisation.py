...
lim = 5.0
ax.set_xlim(-lim, lim)
for i, v in enumerate(np.array(pulls)[order]):
    if abs(v) > lim:                      # mark what runs off the axis
        ax.annotate(f"{v:+.0f}", (np.sign(v) * (lim - 0.15), i),
                    ha="right" if v > 0 else "left", va="center", fontsize=7)
ax.set_xlabel("(recovered - true) / quoted error")
ax.set_yticks([])
n_ok = sum(1 for v in pulls if abs(v) <= 3)
ax.set_title(f"are the error bars honest?  {n_ok} of {len(pulls)} detections "
             "within 3 sigma", fontsize=10)
