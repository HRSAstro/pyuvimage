...
def demo():
    uvd, truth, geom = mock.make_demo_dataset(n_vis=250, seed=2)
...
def test_full_run_recovers_truth(demo, tmp_path):
    ...
    # morphology
    mi = np.nan_to_num(p.model_image)
    corr = np.corrcoef(mi.ravel(), truth.ravel())[0, 1]
    assert corr > 0.85
    # peak position within one mesh pixel
    t = np.unravel_index(np.argmax(truth), truth.shape)
    m = np.unravel_index(np.argmax(mi), mi.shape)
    assert abs(t[0] - m[0]) <= 2 and abs(t[1] - m[1]) <= 2

    # products on disk with WCS + beam + rms headers
    for name in ["model.fits", "dirty_image.fits", "residual.fits",
                 "clean.fits", "pb.fits", "model_pbcor.fits"]:
    ...
    assert h["RMS"] > 0
    assert "BMAJ" in h
    hc = fits.getheader(tmp_path / "out" / "clean.fits")
...
def test_default_prior_is_gibbs(tmp_path):
    ...
    uvd, truth, geom = mock.make_demo_dataset(n_vis=250, seed=5)
    res = pyuvimage.run(
...
def test_no_overfitting_at_high_snr(tmp_path):
    ...
    uvd, truth, geom = mock.make_demo_dataset(n_vis=200, sigma_jy=1e-5, seed=11)
    res = pyuvimage.run(
    ...
    mi = np.nan_to_num(p.model_image)
    assert np.nansum(np.abs(mi[~on])) < 0.6 * np.nansum(np.abs(mi))


def test_products_share_one_grid(tmp_path):
    uvd, truth, geom = mock.make_demo_dataset(n_vis=200, seed=3)
    res = pyuvimage.run(
        uvd, fov=3.0, mesh_shape=geom.mesh_shape, out=tmp_path / "grid"
    )
    shapes = {
        fits.getdata(tmp_path / "grid" / f).shape
        for f in ["model.fits", "dirty_image.fits", "clean.fits",
...
def test_posterior_uncertainty_exceeds_sampling(tmp_path):
    """The posterior width includes the prior's own contribution, so it must
    be at least the noise-only scatter everywhere."""
    uvd, truth, geom, comps = mock.make_extended_plus_compact_dataset(
        n_vis=250, mesh_n=16, sigma_jy=1e-3, seed=42)
    res = pyuvimage.run(uvd, fov=3.0, mesh_shape=(16, 16), out=tmp_path / "u",
                        reg="matern")
    p = res.products[0]
    assert p.uncertainty is not None and p.uncertainty_sampling is not None
    assert np.all(p.uncertainty >= p.uncertainty_sampling - 1e-12)
    for name in ("uncertainty.fits", "uncertainty_noise.fits", "snr.fits"):
        assert (tmp_path / "u" / name).exists(), name
