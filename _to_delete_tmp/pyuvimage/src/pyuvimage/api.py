...
def run(
    ...
    n_data = 2 * len(d)
    if mfs_fit.chi_squared / n_data > 10:
        logger.warning(
            "chi^2/N = %.1f is far above 1: the model cannot reproduce the "
            "data. Common causes: the field of view (--fov) does not cover "
            "all the emission, the pixel scale is too coarse for the S/N, or "
            "the noise map underestimates the true noise.",
            mfs_fit.chi_squared / n_data,
        )
