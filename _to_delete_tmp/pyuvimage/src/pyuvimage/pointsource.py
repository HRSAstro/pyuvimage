...
def _masked_peak(dirty, pixel, excluded, radius):
    """Brightest residual pixel, ignoring a radius around earlier candidates."""
    n = dirty.shape[0]
    amp = np.abs(np.asarray(dirty, dtype=float))
    if excluded and radius > 0:
        iy, ix = np.mgrid[0:n, 0:n]
        yy = ((n - 1) / 2 - iy) * pixel
        xx = (ix - (n - 1) / 2) * pixel
        for (ey, ex) in excluded:
            amp[np.hypot(yy - ey, xx - ex) < radius] = np.nan
    if not np.any(np.isfinite(amp)):
        return None
    iy, ix = np.unravel_index(np.nanargmax(amp), amp.shape)
    return (((n - 1) / 2 - iy) * pixel, (ix - (n - 1) / 2) * pixel)
    ...
    if positions:
        ...
        for _ in range(max_points + 3):
            if len(accepted) >= max_points:
                break
            mesh, amps, _, _ = system.solve(accepted)
            resid = np.asarray(dataset.data) - system.model_visibilities(
                mesh, accepted, amps
            )
            dirty = np.asarray(dirty_imager.dirty_image(resid))
            peak = _masked_peak(dirty, pixel, excluded, min_separation)
            if peak is None:
                break
            trial = accepted + [peak]
            if refine:
