...
def main(argv: list[str] | None = None) -> int:
    ...
    p_fit.add_argument(
        "--reg", default="gibbs",
        choices=["gibbs", "adaptive", "matern", "gaussian", "exponential",
                 "constant"],
        help="source prior. gibbs (default): non-stationary GP whose "
        "correlation length shortens where the source is bright — best for "
        "compact/unresolved emission. adaptive: prior amplitude tracks a "
        "first-pass model — best for extended emission. matern/exponential: "
        "stationary GP (PyAutoLabs' default). gaussian: matern with a spatial "
        "envelope, for sparse visibilities. constant: nearest-neighbour "
        "gradient (rank-deficient; evidence ill-behaved).",
    )
    p_fit.add_argument(
    ...
    p_fit.add_argument(
        "--point-retune", action="store_true",
        help="re-tune the regularisation to chi^2 = N with the points present "
             "(stiffer mesh, smaller point error bars; off by default)",
    )
    p_fit.add_argument(
    ...
    if args.command == "fit":
        ...
        run(
            ...
            point_retune=args.point_retune,
        )
        return 0
    ...
    if args.command == "demo":
        ...
        uvd, truth, _ = make_demo_dataset()
        run(uvd, fov=3.0, out=args.out)
