...
convention: the model is Jy/pixel, dirty/residual/clean images are Jy/beam,
...
class ProductSet:
    ...
    model_image: np.ndarray       # Jy / image pixel, same grid as dirty/clean
    dirty_image: np.ndarray       # Jy / beam
    dirty_model: np.ndarray       # Jy / beam
    residual_sigma: np.ndarray    # (data - model) dirty / rms
    clean: np.ndarray             # Jy / beam
    uncertainty: np.ndarray | None    # 1-sigma posterior, Jy/pixel
    uncertainty_sampling: np.ndarray | None  # 1-sigma noise-only scatter
...
def write_products(
    ...
    def model_with_points(p) -> np.ndarray:
        """Extended model plus each point's flux in its nearest pixel.

        The image grid cannot hold a sub-pixel delta, so model.fits is
        flux-correct but positionally quantised; point_sources.json carries
        the exact fitted positions, and clean.fits places them analytically.
        """
        ...
        n = img.shape[0]
        for pt in p.points:
            row = int(round((n - 1) / 2 - pt.d_dec / geometry.pixel_scale))
            col = int(round((n - 1) / 2 - pt.d_ra / geometry.pixel_scale))
    ...
    model_planes = [to_fits_orientation(model_with_points(p)) for p in products]
    w("model.fits",
    ...
    w("clean.fits", stack("clean"),
      hdr(n_img, geometry.pixel_scale, "Jy/beam", beam=med_beam,
          extra={"RMS": (rms_all, "image-plane rms noise [Jy/beam]")}))
    if products[0].uncertainty is not None:
        w("uncertainty.fits", stack("uncertainty"),
          hdr(n_img, geometry.pixel_scale, "Jy/pixel",
              extra={"ERRTYPE": ("posterior", "sqrt(diag((F+H)^-1)) propagated")}))
        w("uncertainty_noise.fits", stack("uncertainty_sampling"),
          hdr(n_img, geometry.pixel_scale, "Jy/pixel",
              extra={"ERRTYPE": ("sampling", "noise-only scatter, no bias")}))
...
def _summary_png(p: ProductSet, geometry: ImageGeometry, path: Path) -> None:
    ...
    panels = [
        ...
        (p.model_image, "model", "Jy/pixel", "inferno", None),
        (p.clean, "clean", "Jy/beam", "inferno", None),
    ...
    axes[3].set_title(
        f"residual  (peak {rmax:.1f}$\sigma$, rms "
        f"{np.nanstd(resid):.2f}$\sigma$)", fontsize=11
    )
    fig.suptitle(
