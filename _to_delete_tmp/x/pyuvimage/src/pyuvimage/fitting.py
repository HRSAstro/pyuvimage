...
ADAPT_POWER = 1.0    # how strongly the prior width tracks brightness
...
def fit_dataset(
    ...
    if positive_only and prior is None:
        ...
        try:
            free = _chi_squared(
                fit_at(dataset, mesh_shape, reg_kind, 1.0,
                       positive_only=False, **probe_kwargs)
            )
            constrained = _chi_squared(
                fit_at(dataset, mesh_shape, reg_kind, 1.0,
                       positive_only=True, **probe_kwargs)
            )
        except Exception:
            free = constrained = np.nan
        if (
            np.isfinite(free) and np.isfinite(constrained)
            and constrained > max(2.0 * free, free + 2 * len(np.asarray(dataset.data)))
        ):
            logger.warning(
                "the non-negative solver fits much worse than the "
                "unconstrained one (chi^2 %.4g vs %.4g) and is not responding "
                "to the prior; disabling positivity for this fit. The model "
                "may contain small negative values.",
                constrained, free,
            )
            positive_only = False
