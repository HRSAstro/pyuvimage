...
def make_demo_dataset(
    ...
    """A compact demo: exponential source.

    Returns (uvdata, truth, geometry); the truth image is on the geometry's
    grid, so fits must use ``mesh_shape=geometry.mesh_shape`` to compare
    against it pixel-for-pixel.
    """
    fov_rad = fov_arcsec / 206265.0
    uv_max = mesh_n / (2.0 * fov_rad)  # wavelengths
    max_b = uv_max * C_M_S / frequency_hz
    uvw = random_uv_coverage(n_vis, max_baseline_m=max_b, frequency_hz=frequency_hz, seed=seed)
    geom = resolve_geometry(
        fov_arcsec,
        max_baseline_wavelengths=float(
            np.max(np.hypot(uvw[:, 0], uvw[:, 1])) * frequency_hz / C_M_S
