...
| `--noise-chunk` (on `pyuvimage import`) | **600** | Chunk length in seconds for `--noise chunked`. A typical ALMA execution is 1-1.5 h including calibrators, which take 30-40% of it, so only ~45-60 min of target time exists to divide; 600 s leaves 5-6 chunks. Shorter follows the track more closely but measures each sigma from fewer differences (~`1/sqrt(2n)`); useful range roughly 300-1200 s. Differences spanning a calibrator gap are excluded automatically. |
...
| `--noise` (on `pyuvimage import`) | **difference** | How the per-visibility noise is set. MS weights are relative, not absolute, so the *scale* is recomputed from the data in every mode. The modes differ in where the **shape** comes from, because the weight column and time-differencing are blind on different axes — the column is radiometric and cannot see decorrelation or calibration quality (the baseline axis), while differencing pools each baseline's whole track into one number and cannot see elevation (the time axis). `difference`: shape from the data only, one sigma per baseline for the whole track. `chunked`: the same within blocks of the track (`--noise-chunk`, default 1200 s) so sigma can follow elevation, measured from the data so it sees phase noise as well as Tsys; a chunk as long as the track reduces to `difference`. `hybrid`: per-baseline level from the data, time profile from the weights; reduces to `difference` when the weights carry no time dependence, so it cannot do worse. `scaled`: whole shape from the weights. `sigma`: trust `SIGMA` as absolute, and warn. Every import prints the weight-vs-data ratio, the baseline-length trend, and the variation over the track. |
